...
Future orderInProgress(
  BuildContext context,
  String channelName,
  Future Function()? onReschedule,
) async {
      ...
      (message) {
        try {
          ...
          if (rawData != null && rawData is Map<String, dynamic>) {
            ...
            final status = newOrder.status?.toLowerCase();

            // Verifica status e limpa o AppState se necessário
            // if (status == 'Concluido' ||
            //     status == 'Agendado' ||
            //     status == 'Cancelado') {
            //   FFAppState().update(() {
            ...
            // Atualiza o AppState normalmente
            FFAppState().update(() {
              FFAppState().lastOrder = newOrder;
            });

            // Verifica se é reeschedule
            if (newOrder.isReeschedule == true) {
              onReschedule?.call();
            }
          } else {
            throw Exception(
